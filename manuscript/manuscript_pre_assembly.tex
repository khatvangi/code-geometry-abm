...
The model developed here tests that claim in a deliberately minimal way. Agents are exchangeable draws from common initial distributions and have no preassigned enforcement role. No charismatic leader, exogenous shock, or abnormal psychological distribution is needed to produce the core dynamics. Under appropriate parameter combinations, punishment becomes concentrated in a small minority selected endogenously through institutional delegation and capital compounding. The system then moves among four recurrent regimes, quiet, mixed enforcement, collapse, and capture, depending on whether enforcement becomes viable, whether exit remains open, and whether the outside option is progressively degraded.
...
The model initializes $N = 350$ agents on a Barab\'asi--Albert scale-free network. The attachment parameter is $m = \max(2, \text{round}(\log(N + 1)))$, yielding $m = 6$ for $N = 350$. After a successful exit, the model removes 90\% of the exited agent's incident edges, with the number removed determined by rounding; otherwise, agents do not form or sever ties during the simulation.

\subsection{Agent state}

Each agent $i$ carries the following state variables. Internal cultivation is initialized from a normal distribution with mean 0.35 and standard deviation 0.12, and belief position from a normal distribution with mean 0.50 and standard deviation 0.12, both clipped to $[0,1]$. Literalism is drawn from $\operatorname{Beta}(1.5,4.5)$, while rigidity and enforcement propensity are each drawn from $\operatorname{Beta}(2,4)$, also clipped to $[0,1]$. External compliance, utility, enforcement capital, and service begin at zero, and enforcer status begins false.
...
\textbf{Literalism} ($L_i$): a fixed trait representing the agent's orientation toward strict textual interpretation. Drawn from a $\operatorname{Beta}(1.5,4.5)$ distribution at initialization and does not change. Literalism enters only through cadre selection (\S6.6), where it is weighted at 30\% of the suitability score.

\textbf{Enforcement capital} ($\text{cap}_i$): a stock variable initialized at zero. Enforcement capital increases when an agent serves in the enforcement cadre and participates in sanctioning. It compounds over time through patronage allocation (the institutional equivalent of budget growth, seniority, and procedural authority) and decays by 0.5\% per step for both active and exited agents.
...
Institutional delegation is the mechanism that concentrates enforcement in a minority. At fixed intervals, the model selects an enforcement cadre of size $q = \operatorname{round}(\text{enforcer\_quota\_frac} \times N_{\text{active}})$, where $\text{enforcer\_quota\_frac} = 0.08$ (8\% of the active population), with a minimum of one when the fraction is positive. Agents are ranked by a suitability score:
\[
...
The endogenous-delta and dose-response sweeps are each run for 450 time steps. This horizon is sufficient for the system to approach steady behavior in the tested configurations: punishment concentration, exit rate, and enforcement-share trajectories stabilize by approximately step 200, with the remaining horizon serving as a buffer against premature classification. At each step, the controller first updates threat, authority, the patronage budget, punishment parameters, and the orthodoxy target. Agents then update their states, cadre assignments are updated, punishment occurs, patronage is distributed and narrative drift is updated, and agents evaluate exit. System metrics are recorded by the outer simulation loop after the model step. Agent ordering is randomized during agent updates at every step to eliminate order-dependent artifacts.
...
Second, institutional delegation concentrates enforcement. Once active, punishment is monopolized by the endogenously selected enforcement cadre. The top 5\% of agents issue 67 to 92 percent of all punishment depending on the sweep; cadre members issue 92 to 98 percent. This concentration is invariant to $\sigma$, $\pi$, $\delta$, $\eta$, $\alpha$, and $\mu$ across all tested parameter values. It is a structural consequence of delegation and capital compounding, confirmed by the $\bar{y}$ ablation (\S8.3).
...
The model's institutional delegation mechanism selects a cadre of size $q = \operatorname{round}(0.08 \times N_{\text{active}})$, with a minimum of one when the fraction is positive, and, under the monopoly-on-enforcement flag, blocks non-cadre agents from punishing when enforcement activity exceeds a threshold ($A \geq 0.35$). A reviewer could ask whether the concentration result is mechanically assisted by this restriction.
...
A consistent finding across all model versions is the decoupling of enforcement production from population retention. The inward-facing dimensions of code geometry ($\sigma$, $\pi$, delegation, capital compounding) reliably produce enforcement concentration. In every active cell across every sweep, the top 5\% of agents issue 67 to 92 percent of all punishment and endogenously selected cadre members deliver 92 to 98 percent. This concentration is invariant to retention parameters ($\alpha$, $\mu$, $\delta$), to drift parameters ($\eta$), and to initial conditions. It is a structural consequence of delegation and compounding, not a contingent outcome.
...
Network structure begins as a Barab\'asi--Albert scale-free graph and changes only when a successful exit removes 90\% of the exited agent's incident edges. In real systems, enforcement produces richer network reconfiguration: enforcers accumulate preferential connections, dissidents are isolated, and boundary-crossing ties (friendships with outsiders, mixed marriages, secular education) are actively severed by the enforcement apparatus. Code-prescribed social boundary rules (endogamy requirements, residential segregation, prohibitions on external media) constitute a distinct mechanism of exit-cost manipulation that operates through network topology rather than through payoff modification. This broader mechanism requires additional agent-level network dynamics and is reserved for a companion paper.
...
The computational results support a specific causal sequence. First, the joint configuration of $\sigma$ and $\pi$ determines whether enforcement becomes viable at all: low $\sigma$ and low $\pi$ produce quiet regimes, while higher values on either axis activate sanctioning. Second, institutional delegation and capital compounding concentrate punishment in a small minority. Once active, the top 5\% of agents issue the large majority of punishments, and cadre members dominate sanctioning output across parameter settings. Third, this concentrated apparatus can drive degradation of the perceived outside option through endogenous drift, but only when enforcement reaches institutional salience. Fourth, once the outside option is sufficiently degraded, exit collapses. Fifth, with members retained, the enforcement apparatus preserves its own target population, institutional resources, and accumulated capital. The result is not merely active enforcement but durable capture.
