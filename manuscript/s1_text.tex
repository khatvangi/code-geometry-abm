...
% sections print as S1, S2, ... so that cross-references from the main text
% ("Online Resource 1, Section S3") resolve to the printed heading
...
the extended model description, the robustness of the outcome classification,
the continuous surfaces that underlie the discretized results, the privilege
ablation, the endogenous exit-capacity module, and the computational provenance
of every reported quantity. Sections~S1 to~S3 are taken from the frozen model
sources. Sections~S4 to~S7 report quantities computed from the committed run
outputs. Section~S8 states where those outputs and the code that produced them
reside.
...
This section carries the material moved out of the Methods section of the main
text: the measurement justification for the parameter-observable mapping, the
full ODD design-concept narrative, and the fields recorded in the two output
files. Nothing here changes any parameter value, equation, or experimental grid
stated in the main text.

\subsection{Reading the parameter-observable mapping}

The four correspondences given in the Methods are mappings from parameters to
institutional features, not scores for whole traditions. The observational unit
is a historically bounded regime in a specific legal and institutional setting.
...
It does not measure latent sincerity and does not assert that visible conduct
is morally sufficient. It asks whether a regime treats auditable performance as
a usable proxy, and whether it rewards, delegates, and retains the actors
organized around that proxy.

The privilege bundle, summarised in the main text as the standing advantages of
an operative cadre, comprises the cadre quota, the coercive multiplier, the
punishment monopoly, backlash protection, capital gains, patronage, and the
cost discount. Each is specified under Submodels in the main text and each is
removed in the ablation floor.

\subsection{Design concepts in full}

\paragraph{Basic principles.} The model combines three ideas developed
separately in the literature reviewed in the main text: that costly, externally
legible observance screens and coordinates participants; that sanctioning
becomes self-sustaining when it is institutionally rewarded rather than
privately costly; and that the value of the outside option governs whether
tightening produces departure or compliance. Legibility is represented as a
structural property of the code rather than as a property of individuals.
...
controller rules rather than imposed values. No agent begins with an
enforcement role; cadre membership is recomputed each step from accumulated
capital, literalism, and rigidity. The privilege ablation reported in the main
text shows that the concentration of punishment, although not imposed directly,
is largely allocated by the institutional privilege architecture rather than
generated by unprivileged interaction.
...
its network neighbours, updates intrinsic cultivation through a noisy
investment decision, and accumulates or sheds exit intent according to its own
utility, the punishment it has received, its degree, and the membership reward.
...
raises $r_i$ by 0.04 and $b_i$ by 0.03; an unfavourable one lowers them by half
those amounts. Both are clipped to $[0,1]$. Literalism does not change, so the
only trait heterogeneity that persists unchanged across a run is the literalism
draw.
...
\paragraph{Interaction.} Interaction is local and mediated by the network in
the compliance, cultivation, and punishment phases: agents perceive and
sanction network neighbours. Interaction is global through the controller,
which sets the enforcement reward, punishment severity, heresy threshold, and
authority faced by every agent, and which distributes the patronage budget
among enforcers.

\paragraph{Stochasticity.} A single seeded generator drives network generation,
the initial trait draws, the imperfect observation draws, the
compliance-display and punishment probabilities, backlash realisation, the
exit-desire and exit-opportunity draws, edge removal on exit, and the per-phase
shuffling of agent order. Stochasticity represents unmodelled heterogeneity and
the unreliability of social observation. Every reported cell is replicated over
30 seeds.
...
the ranking rule given under Submodels in the main text, with no persistent
membership. The institutional controller is an imposed collective representing
the organisation as a whole; it holds no agent state of its own beyond the
controller variables listed in the main text.
...
on the state left by agents that moved earlier in the same phase. The outer
simulation loop records metrics immediately before each call to the ordered
step, so the committed files contain rows indexed $t=0,\ldots,449$. Exited
agents are excluded from compliance and enforcement; their capital and service
continue to decay and their belief follows a small random walk. After a threat
shock the controller may raise $\pi_t$ above its experimental baseline, by the
schedule given under the threat controller in the main text.

Run-level outputs and aggregated CSVs are committed at the paths named in the
main text; Section~S8 gives the full inventory.
