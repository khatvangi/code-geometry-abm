...
% JASSS house style: APA author-date with NO comma between author and year, e.g. (Allen 1997)
\setcitestyle{aysep={}}
...
The remainder of the paper develops the argument in four steps. The next section defines code geometry and derives its structural regimes. The methods section specifies the agent-based design, the parameter-observable mapping, and the independent interventions on activation, exit, and privilege. The results section presents the activation frontier, exit-closure dose-response, and privilege ablation. The discussion states the empirical implications, limits, and tests that follow from separating selection into a group from members' capacity to leave it.
...
The dimensions are specified in advance rather than recovered from simulation. They are drawn from features that recur across documented enforcement regimes: whether conduct is observable enough to be judged, whether enforcing carries a reward, whether the authority to sanction is delegated to a designated few, and whether members can leave. The model does not identify which structural properties matter; it establishes what follows once these are taken as the governing properties. The division of labour is therefore explicit: the framework carries the theoretical commitment, and the model carries the deduction. A reader who doubts the dimensions is doubting the framework rather than the derivation, and the place to settle that doubt is the historical record rather than a further simulation.
...
The interaction of these dimensions generates distinct structural regimes. Four are distinguished here; each has historical correlates, and each is realised in the simulations reported below.
...
The model is documented below in ODD protocol order. Online Resource~1,
Section~S3, carries the extended model description: the full design-concept
narrative, the recorded output fields, and the measurement justification.
...
enforcement into durable capture. Three patterns evaluate it: whether an
apparatus forms, measured as step-level punishment activation among active
agents; whether it retains members, measured as the terminal exit rate; and
whether sanctioning concentrates, measured as the top-share and the Gini of
punishment counts. The model is adequate to the extent that these patterns
respond to distinct structural inputs rather than moving together.

\subsection{Entities, state variables and scales}

The entities are individual agents embedded in a social network and one
system-level institutional controller. The enforcer cadre is a collective
defined over agents rather than a separate entity.
...
disengagement flag, an exited flag, and a network position. Literalism is fixed;
the remaining quantities change during a run. The controller carries a threat
level \(T_t\), an authority level \(A_t\), a patronage budget, the current
enforcement reward and punishment severity, the heresy-distance threshold, the
orthodox belief position, and the current outside-option degradation
\(\delta_t\).

Time is discrete and one step is the only temporal unit; the model makes no
claim about the calendar duration a step represents. Space is not represented.
Each reported run begins with \(N=350\) agents and proceeds for 450 steps, and
each cell uses 30 independent seeds, numbered 1 through 30.

\subsection{Parameter-observable mapping}

Four correspondences connect model inputs to institutional features.
...
promotion, resources, or authority attached to successful sanctioning. The
privilege bundle represents the standing advantages of an operative cadre, and
exit conditions represent the relational, economic, legal, and cultural
resources that make departure practicable. Online Resource~1, Sections~S1
to~S3, supplies the indicator table, parameter defaults and tested ranges, and
the extended measurement justification.

\subsection{Process overview and scheduling}

At the start of a step the controller applies or relaxes threat and, with it,
authority, the patronage budget, punishment reward and severity, and the heresy
threshold; unless orthodoxy is fixed, it also updates the orthodox belief
position. Agents are then shuffled and update their states. The model
next recomputes mean literalism among active agents, clears and reselects the
cadre, resets punishment counters, determines whether monopoly is active, and
executes the shuffled punishment phase. It then computes active-agent punishment
incidence, distributes patronage, and updates \(\delta_t\). Agents are finally
reshuffled and attempt exit; successful exits remove edges, and the clock is
incremented after exit. Metrics are recorded
immediately before each ordered step, so the committed files contain rows
indexed \(t=0,\ldots,449\).

Agent order is randomised afresh within each phase, and updates within a phase
are asynchronous: an agent acts on the state left by agents that moved earlier.

\subsection{Design concepts}

Online Resource~1, Section~S3, gives the full narrative for the concepts
summarised here.

\paragraph{Basic principles.} Legibility is a property of the code rather than
of individuals, sanctioning is institutionally rewarded rather than privately
costly, and the outside option governs whether tightening produces departure or
compliance.

\paragraph{Emergence.} Activation, the terminal exit rate, and the distribution
of punishment are outcomes of agent and controller rules; no agent begins with
an enforcement role, and cadre membership is recomputed each step.

\paragraph{Adaptation.} Agents adapt without optimising: compliance display
responds to local norms, cultivation to a noisy investment decision, and exit
intent to utility, punishment received, degree, and the membership reward.

\paragraph{Objectives.} No agent maximises an objective function; accumulated
utility enters probabilistic decision rules, principally the exit-desire
logistic.

\paragraph{Learning.} A favourable punishment outcome raises \(r_i\) by 0.04 and
\(b_i\) by 0.03, an unfavourable one lowers them by half those amounts, and both
are clipped to \([0,1]\). Literalism does not change.

\paragraph{Prediction.} Agents make no forecast and decide on current local
information alone.

\paragraph{Sensing.} Neighbours are observed imperfectly: a compliance marker
with probability \(v_{\mathrm{obs}}=0.90\), cultivation with
\(a_{\mathrm{obs}}=0.05\), and a belief position, for the heresy-distance term,
with \(h_{\mathrm{obs}}=0.15\). The controller senses population-level
quantities directly.

\paragraph{Interaction.} Interaction is network-mediated in the compliance,
cultivation, and punishment phases, and global through the controller, which
sets reward, severity, heresy threshold, and authority and distributes
patronage.

\paragraph{Stochasticity.} One seeded generator drives network generation,
initial traits, observation, compliance and punishment probabilities, backlash,
the exit draws, edge removal, and per-phase shuffling.

\paragraph{Collectives.} The enforcer cadre is emergent and reselected every
step with no persistent membership; the controller is imposed and holds no agent
state beyond the variables listed above.

\paragraph{Observation.} Each run writes a step-level file of population
quantities and an agent-level file of terminal agent records.

\paragraph{Outcome classification.} The classifier treats activation and
retention as separate axes: a run is active when \texttt{max\_punish} is at
least 0.10 and retained when \texttt{final\_exit\_rate} is at most 0.20, so
\begin{equation}
 \mathrm{CAPTURE}=\mathbf{1}\{\mathrm{final\_exit\_rate}\leq0.20\}\,\mathbf{1}\{\mathrm{max\_punish}\geq0.10\}.
\end{equation}
Inactive runs are QUIET, active and not-retained runs are MIXED, and active and
retained runs are CAPTURE. COLLAPSE, defined descriptively by exit at least
0.90, is a tail within the inactive region: 100 percent of committed collapse
runs have \texttt{max\_punish} below 0.10. The joint robustness band for the
activation threshold is \([0.075,0.15]\), so continuous surfaces are the primary
results and the four-class discretization a secondary summary.

\paragraph{Concentration and incidence metrics.} Punishment incidence is
computed only among active agents: \texttt{max\_punish} is the maximum across
recorded steps of the fraction of then-active agents punished at least once in
that step, and \texttt{final\_exit\_rate} is the terminal exited fraction of the
original population. For the privilege ablation, let \(c_i\) be cumulative
punishments issued by agent \(i\) among the \(n_a\) agents active at the end of
the run. Then
\begin{equation}
 \mathrm{top5pct\_share\_active}=\frac{\sum_{i\in\mathcal{K}}c_i}{\sum_{i=1}^{n_a}c_i},\qquad |\mathcal{K}|=\max\{1,\lceil0.05n_a\rceil\},
\end{equation}
where \(\mathcal{K}\) contains the largest counts, and with \(c_{(i)}\) sorted
ascending the active-agent Gini is
\begin{equation}
 \mathrm{gini\_active}=\frac{n_a+1-2\sum_{i=1}^{n_a}\left(\sum_{j=1}^{i}c_{(j)}\right)/\sum_{j=1}^{n_a}c_{(j)}}{n_a},
\end{equation}
set to zero for an empty or zero-total vector. The random-allocation null holds
the observed volume and \(n_a\) fixed, allocates events multinomially with equal
probability \(1/n_a\), repeats this 100 times using the run seed, and records
the median simulated Gini. \texttt{enforcer\_punish\_share} is descriptive only:
it saturates near 0.97 wherever punishment occurs.

\subsection{Initialisation}

Initialisation is driven by the run seed, so it varies across replicates but not
across conditions at a given seed. Intrinsic
cultivation is \(x_i\sim\operatorname{clip}_{[0,1]}\{\mathcal{N}(0.35,0.12^2)\}\),
belief position is
\(y_i\sim\operatorname{clip}_{[0,1]}\{\mathcal{N}(0.50,0.12^2)\}\), literalism
is \(L_i\sim\operatorname{Beta}(1.5,4.5)\), and rigidity and enforcement
propensity are drawn independently as
\(b_i,r_i\sim\operatorname{Beta}(2,4)\). The compliance marker begins at
\(s_i=0\); utility, capital, and service begin at zero; and all agents enter as
non-exited members without an enforcement role.

The reported sweeps use a Barab\'asi--Albert scale-free network with attachment
parameter
\begin{equation}
 m=\max\{2,\operatorname{round}[\log(N+1)]\}=6
\end{equation}
for \(N=350\), generated from the run seed. The controller begins with threat,
and therefore authority, at zero, the patronage budget at its constant term, and
the closure level at the cell's imposed baseline \(\delta_0\).

\subsection{Input data}

The model uses no external input data. Every driver is endogenous to the agent
and controller rules or fixed by a parameter for the run, including the threat
schedule, which is a list of step indices rather than an empirical series. No
reported result is calibrated to observations. The only file read at launch is
the calibration parameter set committed at
\path{results/BEST_PARAMS_frozen.json}, whose contents are given in Online
Resource~1.

\subsection{Submodels}

\paragraph{Perception and the code-geometry parameter.} The code-geometry
parameter \(\sigma\in[0,1]\) weights externally legible compliance against
intrinsic cultivation in the perceived-morality signal,
\begin{equation}
 \widehat m_{ij}=(1-\sigma)\widehat x_{ij}+\sigma\widehat s_{ij}.
\end{equation}
Failed compliance observation is coded as zero; failed cultivation observation
is replaced by a draw from
...
\end{equation}
and \(\pi\) is the coefficient on this index, so with effective enforcement cost
\(\kappa_{i,t}^{\mathrm{eff}}\) the punisher's net institutional return is
\begin{equation}
 \Delta u_{i,t}^{\mathrm{pun}}=\pi_t\ell(\sigma)-\kappa_{i,t}^{\mathrm{eff}}.
\end{equation}

\paragraph{Agent update.} An active agent chooses visible compliance
probabilistically from local compliance and enforcement norms, updates
cultivation by a noisy investment decision, and drifts weakly toward the current
orthodoxy. Each active agent also receives a per-step membership reward
\begin{equation}
 g_{i,t}=0.0634+0.08\,\ell(\sigma)+0.1055\,T_t,
\end{equation}
which is added to utility and therefore offsets exit desire. Capital and service
decay every step at rates 0.005 and 0.02. Exited agents are excluded from
compliance and enforcement, and their capital and service continue to decay.

\paragraph{Exit.} Exit has distinct desire and opportunity components, with
\texttt{exit\_opportunity\_base} \(=0.30\) and \texttt{exit\_threshold}
\(=-1.0\) in all reported experiments. With degree \(d_{i,t}\), threat \(T_t\),
and current outside-option degradation \(\delta_t\), the raw opportunity is
\begin{align}
...
\end{align}
In legacy and exogenous modes \(p^{\mathrm{opp}}_{i,t}=\widetilde
p_{i,t}(1-\delta_t)\) when \(\delta_t>0\); in decoupled mode the outer factor is
not applied. Exit desire is a logistic function of the threshold minus
accumulated utility, punishment received, degree, and membership reward, must
accumulate for twelve steps, and must then be matched by a successful
opportunity draw. Effective exit cost is \(1.00(1+0.03d_{i,t})+0.25s_i\). A
successful exit removes \(\operatorname{round}(0.90d_i)\) of the agent's
incident edges, so the network is not fixed.

\paragraph{Outside-option closure.} Let \(\delta_0\) denote the imposed baseline
and \(\eta\) the drift rate, clipped by the code to \([0,0.5]\). Drift updates
only when the step-level fraction of active agents punished is at least 0.08,
and conditional on that gate
\begin{equation}
 \delta_{t+1}=\operatorname{clip}_{[0,1]}\left\{\delta_t+\eta(\delta_t^{*}-\delta_t)\right\}.
\end{equation}
Exit capacity is an independent structural input, since closure driven by the
enforcement activity it is meant to explain would yield capture by construction.
In that specification
\begin{equation}
 \delta_t^{*}=\min\{\delta_{\max},\delta_0+\kappa q_t\},
\end{equation}
with \(q_t\) the active-agent punishment incidence, \(\delta_{\max}=0.85\), and
\(\kappa\) varied by the experiment. In exogenous mode no drift occurs and
\(\delta_t=\delta_0\) throughout.

\paragraph{Cadre selection.} Among active agents the target cadre size is
...
eligible first and are ranked by
\begin{equation}
 z_i=0.6\,\mathrm{cap}_i+0.3L_i+0.1(r_ib_i).
\end{equation}
Remaining positions are filled by the highest-ranked ineligible agents.

\paragraph{Privilege bundle.} The bundle combines exclusivity, coercive
advantage, reduced cost and risk, and material reinforcement. When controller
authority \(A_t\geq0.35\), the punishment monopoly excludes all non-enforcers
from policing. Enforcers carry a punishment-probability multiplier
of 1.5 whether or not monopoly is active; with monopoly off, non-enforcers
receive the operative multiplier \texttt{non\_enforcer\_punish\_eps} \(=0.02\),
while the separately declared \texttt{non\_enforcer\_punish\_mult} \(=0.25\) is
passed by the sweeps but unused in this branch. Capital first reduces the base
cost according to
\begin{equation}
...
Enforcer backlash probability is multiplied by 0.25. Every successful punishment
adds 0.15 capital, capped at 2.0, and one unit of service. The nonnegative
budget \(0.15+0.60T_t\) is distributed only among active enforcers, equally if
their total service is zero and otherwise in proportion to nonnegative
accumulated service.
...
all decoupled runs use an empty schedule, so threat is identically zero and
every threat-gated term vanishes.
...
off condition has no shocks; the on condition uses the schedule above.
...
0.05,\allowbreak 0.10,\allowbreak 0.15,\allowbreak 0.25,\allowbreak 0.50\}\),
and 30 seeds. Both use exogenous mode and \(\eta=0\); the open boundary fixes
\(\delta_0=0\), and the sealed boundary fixes \(\delta_0=0.95\).

The privilege ablation is committed at
\path{recon/privilege_ablation/ablation_seed_results.csv}. The floor and ceiling
each contain 120 runs crossing
\((\sigma,\pi)\in\{(0.75,0.25),\allowbreak (0.75,0.50),\allowbreak
(0.95,0.25),\allowbreak (0.95,0.50)\}\) with 30 seeds, both at \(\delta_0=0.95\)
in exogenous mode with no drift and no shocks. The floor disables monopoly and
the cadre quota, equalizes punishment multipliers at 1.0, removes capital gain,
capital and role cost discounts, and backlash protection, and sets the patronage
budget to zero. The ceiling retains the complete default bundle described above.

\subsection{Reproducibility}

The implementation is written in Python using Mesa 3.4.2 \citep{mesa2020}, NumPy
2.4.2 \citep{numpy2020}, pandas 3.0.0 \citep{pandas2010}, and NetworkX 3.6.1
\citep{networkx2008}, with versions pinned in \path{requirements.txt}. The seven
parameters that the sweep driver reads at launch from
\path{results/BEST_PARAMS_frozen.json} are listed in Online Resource~1,
Section~S2, and the values stated above are those values. They were confirmed by
replaying single runs from three sweeps against the frozen model source and
reproducing the committed \path{metrics.csv} and \path{agent_summary.csv}
exactly.

The canonical classifier in \path{src/regime_classifier.py} is the single source
of truth for discrete outcomes. All reported regimes are computed from
continuous \texttt{final\_exit\_rate} and \texttt{max\_punish}; stored regime
columns are non-authoritative.
...
equal: its Gini of 0.240 exceeds the random-allocation null of 0.028. That residual is the
...
Imposed closure can convert activated regions to capture, but cannot manufacture capture where enforcement lacks both
legibility and reward. The dose-response therefore establishes that exit closure is necessary but not sufficient within
the tested design. Exit capacity is an independent structural condition in the main intervention, while the
internal-absorption extension updates capacity without enforcement inputs.

These results are entailments of a specification rather than discoveries about which dimensions
govern enforcement. The dimensions are fixed before any run, so nothing in the runs can vindicate
the choice of them. Whether they are the right dimensions is settled by the historical record that
motivated them and by the predictions they generate, not by further simulation. The predictions set
out below are therefore addressed to cases that played no part in specifying the model.

\subsection{Enforcement without intention}

The account is structural in the strict sense. No agent in it holds a doctrine, believes anything,
or intends domination. Agents occupy positions defined by four conditions: how visible their conduct
is, what enforcing returns to them, whether the authority to sanction is delegated to a designated
few, and whether an outside option remains available. Enforcement concentrates in a small cadre and
populations lock in without any actor preferring either outcome. The pattern is a property of the
arrangement of positions, not of the dispositions of those who occupy them.

This cuts against the intentionalist account, which explains such regimes by the character of the
people who staff them: zealots, true believers, opportunists. Zeal is not required. A position that
combines visibility, institutional return, delegated authority, and foreclosed exit produces
concentrated sanctioning whoever fills it. What a content-based explanation cannot easily
accommodate is the recurrence of similar enforcement forms across traditions that share no doctrinal
content; a positional account predicts that recurrence, because the same positions can be assembled
from any doctrine. Conviction and enthusiasm are present in historical regimes and shape their
texture, but they are not what generates the form.
...
The framework also speaks to contemporary settings, though what it speaks to is the durability of
enforcement rather than the mobilisation that produces it. Where exit capacity is high, so that
departure carries no legal penalty, livelihoods do not run through the community, and members retain
dense ties beyond it, the account predicts active but leaky enforcement rather than durable control.
Enforcement intensity should therefore not translate into retention: rising enforcement activity
should be observed alongside continuing disaffiliation, and the two should be measured separately
rather than read off one another. Lock-in, where it occurs, should be found not in the movements
that enforce most visibly but in subcommunities where exit is locally foreclosed. The indicators of
that foreclosure are relational and material rather than doctrinal: outside ties severed rather than
merely thinned; employment, housing, and schooling routed through the community; children raised
without sustained relationships beyond it. Retention should track exit foreclosure rather than
enforcement intensity, so a design that samples on visible enforcement will mistake activity for
durability. The informative negative cases are demanding communities under open exit, where
requirements are heavy and departure remains practicable, and any serious test must include them.
These are applications of the framework rather than results, since no contemporary case has been
scored or simulated here.
...
All simulation code, data, and analysis scripts are available for review: a copy of the model, with its ODD documentation, the frozen calibration parameters, and the committed sweep summaries, is archived at the CoMSES Net Computational Model Library, \url{https://www.comses.net/codebase-release/9ab5795b-836b-4519-a9a1-16d3d06e400d/}. Upon acceptance the model will be published with a permanent handle and the version-control repository linked.

\section*{Supporting Information}

\noindent\textbf{Online Resource 1.} Supporting text covering the parameter-to-observable mapping, the complete model-parameter tables, the extended model description with the full ODD design-concept narrative and the recorded output fields, classification robustness including the activation-threshold sensitivity, the continuous observability--reward and closure surfaces, the privilege ablation, the endogenous exit-capacity analysis with its anti-circularity test, and computational reproducibility with the committed-run inventory.
