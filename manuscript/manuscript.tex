...
\author{Boggavarapu Kiran$^{1}$}

\date{}

\newcommand{\affiliation}{$^{1}$Department of Chemistry and Physics, McNeese State University, Lake Charles, LA 70607. Email: kiran@mcneese.edu}
...
\begin{abstract}
Enforcement-dominant dynamics in codified moral systems are often explained through extremists, charismatic leaders, or exogenous political shocks. This paper develops an alternative account: such dynamics can arise endogenously from the structural geometry of the code itself. We formalize five dimensions of code geometry, namely legibility, substitutability, enforcement affordance, centralization, and exit cost, and test their consequences in an agent-based model with symmetric agents, no pre-assigned enforcement roles, and 3,246 simulations across five sweeps. The model generates four recurrent regimes: quiet, mixed enforcement, collapse, and capture. Results show that enforcement activation is governed not by legibility alone but by a joint boundary between legibility and enforcement reward: low values on both suppress enforcement, while higher values open a stable enforcement niche. Once active, institutional delegation and capital compounding concentrate punishment in an endogenously selected minority. Two plausible retention mechanisms, sanctified suffering and membership reward, fail across their tested ranges to convert mixed enforcement into durable capture. By contrast, endogenous degradation of the perceived outside option sharply reduces exit and broadens the capture region. Ablation shows that punishment concentration does not depend on orthodoxy drift or monopoly artifacts, but on delegation and compounding enforcement capital. The main implication is structural: enforcement concentration and minority capture need not be imported into codified systems from outside. Under identifiable conditions, they are equilibrium outcomes of the system itself.
\end{abstract}
...
\begin{table}[ht]
\centering
\caption{Illustrative code-geometry scores for historically bounded regimes. Scores are 0--4 ordinal; full anchor definitions in S1 Text.}
\label{tab:scores}
...
The illustrative regime scores provided in S1 Text are therefore not offered as final judgments on civilizational traditions. They are demonstrations of application under a protocol whose validity depends on reproducibility, disagreement tracking, and exposure to disconfirming cases. Their purpose is methodological: to show how the rubric operates, what kinds of evidence it uses, and what kinds of comparative claims it can and cannot support at the present stage.
...
A run is classified as COLLAPSE when exit rate is at least 0.90, meaning that fewer than 10\% of the initial population remain active. A run is classified as CAPTURE when two conditions hold jointly: fundamentalist prevalence is at least 0.90 and exit rate is at most 0.20. This operational definition is intentionally strict. It reserves the capture label for cases in which enforcement dominance is accompanied by durable retention rather than by transient tightening or partial depopulation. A run is classified as MIXED when enforcement is active, as indicated by maximum per-capita punishment of at least 0.10, but neither collapse nor strict capture criteria are met. QUIET is the residual category: punishment remains below the active-enforcement threshold and the population does not undergo mass exit.
...
The confirmatory baseline sweep, spanning 72 parameter cells across $\sigma$, $\pi$, base opportunity, and exit threshold with five seeds per cell, produces four recurrent outcome classes under the operational regime classifier: quiet, mixed enforcement, collapse, and capture. These outcomes occupy distinct regions of parameter space rather than forming a single linear continuum. What varies across the space is not merely the amount of punishment, but the qualitative relation among enforcement viability, exit, and retention.

Quiet cells are characterized by low punishment and low exit. Median maximum per-capita punishment is 0.066 (range 0.036 to 0.091), and median exit rate is 0.096. These cells occupy the low-legibility, low-enforcement-reward region of the sweep. At $\sigma = 0.25$ and $\pi = 0.05$, all cells remain quiet regardless of exit cost. Under these conditions, visible compliance is too weak a proxy for moral standing, and enforcement remains too weakly rewarded, for sanctioning to stabilize as a social role.

Mixed-enforcement cells exhibit active punishment together with substantial but incomplete exit. Median maximum per-capita punishment rises to 0.116, while median exit rate rises to 0.359. These are not failed capture states in the trivial sense. They are a distinct regime in which enforcement is viable, punishment becomes concentrated, and visible compliance rises, but the system cannot fully close the exit channel. The result is a hardened residual community rather than total institutional dominance.

Collapse cells are defined by near-total depopulation. Median exit rate is 0.993, and all collapse outcomes in the baseline occur at $\sigma = 0.25$ under easy-exit conditions. Here the system crosses the activation threshold for social friction but not the retention threshold for enforcement stabilization. Tightening therefore destroys the community rather than consolidating it. This distinction matters because it shows that punishment alone is not enough to generate durable fundamentalist equilibrium. Exit structure remains decisive.

The baseline capture regime appears only in a single extreme cell, at $\sigma = 0.95$, $\pi = 0.50$, base opportunity $= 0.30$, and exit threshold $= -1.00$. This result should not be oversold. In the exogenous baseline specification, capture is possible but occupies only a narrow corner of the parameter space. High legibility and high enforcement reward are necessary but not by themselves sufficient for broad capture. What they reliably generate is enforcement viability. Whether that viability becomes durable domination depends on the retention problem analyzed below.
...
Because many drift-on runs exhibit near-zero exit, saturated $\delta$, and maximal enforcer domination while remaining below the prevalence cutoff for formal CAPTURE classification, the continuous behavioral metrics are more informative here than the categorical label alone.
...
At $\eta > 0$ (drift enabled, 1080 runs): 120 quiet, 494 mixed, 466 capture. Capture emerges broadly. In all 466 capture runs, final $\delta = 1.0$ (the maximum), median exit rate $= 0.006$, and median enforcer punishment share $= 0.981$. The enforcement apparatus drove $\delta$ from initial values of 0.1 to 0.3 all the way to 1.0 endogenously.

The critical test: 32 cells meet the strict low-initial-delta capture criterion ($\delta_0 \in \{0.1, 0.2\}$, $\eta > 0$, $\sigma \geq 0.75$, $\geq 70\%$ of seeds classifying as capture). These cells were initialized with mild outside-world degradation (the outside is ``somewhat worse,'' not ``catastrophic'') and the enforcement apparatus drove them to maximum degradation through the feedback loop. At the strongest capture cells ($\sigma = 0.95$, $\pi = 0.25$, $\eta = 0.30$), 10 of 10 seeds capture with median exit rate 0.003.
...
\caption{Regime map of the baseline parameter sweep.}
\label{fig:2}
\end{figure}
...
\caption{Enforcement concentration across active-regime cells.}
\label{fig:3}
\end{figure}
...
All simulation code, data, and analysis scripts are publicly available at:
\texttt{https://github.com/khatvangi/code-geometry-abm}
