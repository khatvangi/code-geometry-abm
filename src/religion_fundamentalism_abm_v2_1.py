...
class Person(Agent):
    ...
    def police_phase(self):
        ...
        for tid in targets:
            ...
            h_def = sigmoid(p.k_h * (d - p.d0))

            theta = 0.55 - 0.20 * self.b
            deficit = p.w_m * (theta - m_hat) + p.w_heresy * h_def
            p_pun = self.r * sigmoid(p.beta_pun * deficit)
    ...
    def maybe_exit(self):
        ...
        p_exit = sigmoid(p.exit_prob_slope * exit_drive)
        if rng.random() >= p_exit:
            self.exit_cost_eff_last = max(
                p.exit_cost * (1.0 + p.exit_degree_coeff * deg) + p.exit_epc_lockin_coeff * self.s,
                p.exit_min_cost,
            )
            return
...
def parse_args():
    ...
    ap.add_argument("--exit_prob_slope", type=float, default=6.0)
