...
class Params:
    ...
    budget_base: float = 0.02
    budget_threat_gain: float = 0.08
    enforcer_quota_frac: float = 0.08
    cap_decay: float = 0.01
    cap_gain_per_punish: float = 0.06
...
class Person(Agent):
    ...
    def police_phase(self):
        ...
        for tid in targets:
            ...
            if self.is_enforcer:
                pun_mult = 1.0 + self.model.A * (p.enforcer_punish_mult - 1.0)
            else:
                pun_mult = 1.0 - self.model.A * (1.0 - p.non_enforcer_punish_mult)
...
class ReligionABM(Model):
    ...
    def metrics(self) -> Dict[str, Any]:
        ...
        ys = np.array([a.y for a in self.people.values()], dtype=float)
        Ls = np.array([a.L for a in self.people.values()], dtype=float)
        caps = np.array([a.cap for a in self.people.values()], dtype=float)
        services = np.array([a.service for a in self.people.values()], dtype=float)
        ...
        exited = np.array([a.exited for a in self.people.values()], dtype=bool)
        exit_intent = np.array([a.exit_intent for a in self.people.values()], dtype=float)
...
def parse_args():
    ...
    ap.add_argument("--budget_base", type=float, default=0.02)
    ap.add_argument("--budget_threat_gain", type=float, default=0.08)
    ap.add_argument("--enforcer_quota_frac", type=float, default=0.08)
    ap.add_argument("--cap_decay", type=float, default=0.01)
    ap.add_argument("--cap_gain_per_punish", type=float, default=0.06)
