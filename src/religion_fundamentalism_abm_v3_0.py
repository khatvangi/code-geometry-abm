...
class Params:
    ...
    # v3.0 — argument-template input (Phase S2 Extension 1)
    # T_FREE = no template, set sigma/pi/etc. directly as in v2.5 (back-compat).
    # Otherwise validates user-supplied params against the §3.3 envelope.
    argument_template: str = "T_FREE"


# v3.0 envelope ceilings — sourced verbatim from
# synthesis/INSTRUCTION_BORON_S2.md §3.3 (verified to reproduce
# from §3.2 formula + §3.2.1–§3.2.4 semantic rules).
# (sigma_max, pi_max, base_opp_floor, exit_cost_baseline)
ENVELOPE_TABLE: Dict[str, Tuple[float, float, float, float]] = {
    "T1": (0.65, 0.15, 0.30, 0.70),  # exclusive_salvation     C, A, O, U
    "T2": (0.65, 0.15, 0.30, 0.70),  # final_sorting           C, A, U, O
    "T3": (0.40, 0.25, 0.30, 0.30),  # expansion_mandate       M, U, A
    "T4": (0.45, 0.45, 0.70, 0.70),  # purification            C, M, E, O, R
    "T5": (0.40, 0.40, 0.30, 0.30),  # sacred_enforcement      M, E, R, A
    "T6": (0.55, 0.15, 0.30, 0.70),  # softened_modern (P caps)
    "T7": (0.20, 0.05, 0.30, 0.30),  # counter_reading         P only
    "T8": (0.45, 0.25, 0.30, 0.70),  # asymmetric_rights       T8 override
}
VALID_TEMPLATES = ("T_FREE",) + tuple(sorted(ENVELOPE_TABLE.keys()))


def validateEnvelope(params: Params) -> None:
    """validate user-supplied sigma/pi/base_opp/exit_cost against template envelope.

    when argument_template == 'T_FREE' this is a no-op (back-compat with v2.5
    sweeps). otherwise raise ValueError naming the offending parameter.

    constraints (per INSTRUCTION_BORON_S2.md §3.3):
        sigma            <= sigma_max
        pi_reward        <= pi_max
        exit_opportunity_base >= base_opp_floor
        exit_cost        >= exit_cost_baseline
    """
    t = params.argument_template
    if t == "T_FREE":
        return
    if t not in ENVELOPE_TABLE:
        raise ValueError(
            f"argument_template={t!r} is not one of {VALID_TEMPLATES}"
        )
    sigma_max, pi_max, base_opp_floor, exit_cost_baseline = ENVELOPE_TABLE[t]
    if params.sigma > sigma_max + 1e-9:
        raise ValueError(
            f"argument_template={t}: sigma={params.sigma} exceeds envelope "
            f"sigma_max={sigma_max}. raise the template ceiling or pick a "
            f"different template."
        )
    if params.pi_reward > pi_max + 1e-9:
        raise ValueError(
            f"argument_template={t}: pi_reward={params.pi_reward} exceeds "
            f"envelope pi_max={pi_max}."
        )
    if params.exit_opportunity_base < base_opp_floor - 1e-9:
        raise ValueError(
            f"argument_template={t}: exit_opportunity_base="
            f"{params.exit_opportunity_base} below envelope "
            f"base_opp_floor={base_opp_floor}."
        )
    if params.exit_cost < exit_cost_baseline - 1e-9:
        raise ValueError(
            f"argument_template={t}: exit_cost={params.exit_cost} below "
            f"envelope exit_cost_baseline={exit_cost_baseline}."
        )
...
class ReligionABM(Model):
    def __init__(self, params: Params):
        # v3.0: validate envelope BEFORE running. raises ValueError on violation.
    ...
    def metrics(self) -> Dict[str, Any]:
        ...
        return dict(
            ...
            # v2.5.1: denominator restricted to active (non-exited) agents
            punish_rate=float((punished[~exited] > 0).mean()) if (~exited).any() else 0.0,
            # v3.0: explicit active-only count metric (Phase S2 §4.1.3).
            # punished is per-step count of punishment acts received.
            # active_punish_rate = mean acts per active agent. complementary to
            # punish_rate (which is incidence among active agents).
            active_punish_rate=float(punished[~exited].mean()) if (~exited).any() else 0.0,
...
def parse_args():
    ...
    ap.add_argument(
        ...
        help="v3.0: select an argument-template envelope (T1-T8) or T_FREE for v2.5 back-compat. "
             "Non-T_FREE values validate sigma/pi_reward/exit_opportunity_base/exit_cost against the "
             "template envelope per INSTRUCTION_BORON_S2.md §3.3.",
    )
...
def main():
    ...
    params = Params(
        ...
        argument_template=args.argument_template,
    )
