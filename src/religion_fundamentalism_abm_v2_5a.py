#!/usr/bin/env python3
import os, math, argparse, sys, json, hashlib, subprocess
...
class Person(Agent):
    ...
    def police_phase(self):
        ...
        for tid in targets:
            ...
            if monopoly_on:
                pun_mult = p.enforcer_punish_mult if self.is_enforcer else 0.0
            else:
    ...
    def maybe_exit(self):
        ...
        exit_drive = (p.exit_threshold - self.u) + 0.5 * self.punished - 0.02 * deg
...
class ReligionABM(Model):
    ...
    def _update_cadre_assignments(self):
        ...
        if len(eligible) >= quota:
            ...
            eligible_ids = {id(a) for a in eligible}
            # Cap-first fill for below-threshold agents; L is only a tie-breaker.
            remainder = sorted(
                [a for a in active if id(a) not in eligible_ids],
                key=lambda a: (a.cap, a.L),
                reverse=True,
            )
            chosen = list(eligible) + remainder[:shortfall]
        for a in chosen:
            a.is_enforcer = True
    ...
    def metrics(self) -> Dict[str, Any]:
        ps = self.params
        all_agents = list(self.people.values())
        active_agents = [a for a in all_agents if not a.exited]

        xs = np.array([a.x for a in all_agents], dtype=float)
        ys = np.array([a.y for a in all_agents], dtype=float)
        Ls = np.array([a.L for a in all_agents], dtype=float)
        caps = np.array([a.cap for a in all_agents], dtype=float)
        services = np.array([a.service for a in all_agents], dtype=float)
        ss = np.array([a.s for a in all_agents], dtype=float)
        rs = np.array([a.r for a in all_agents], dtype=float)
        bs = np.array([a.b for a in all_agents], dtype=float)
        Fs = rs * bs
        punished = np.array([a.punished for a in all_agents], dtype=float)
        exited = np.array([a.exited for a in all_agents], dtype=bool)
        exit_intent = np.array([a.exit_intent for a in all_agents], dtype=float)

        if active_agents:
            xs_a = np.array([a.x for a in active_agents], dtype=float)
            ys_a = np.array([a.y for a in active_agents], dtype=float)
            ss_a = np.array([a.s for a in active_agents], dtype=float)
            rs_a = np.array([a.r for a in active_agents], dtype=float)
            bs_a = np.array([a.b for a in active_agents], dtype=float)
            Fs_a = rs_a * bs_a
            mean_x_active = float(xs_a.mean())
            mean_y_active = float(ys_a.mean())
            mean_epc_active = float(ss_a.mean())
            mean_r_active = float(rs_a.mean())
            mean_b_active = float(bs_a.mean())
            fund_prevalence_active = float((Fs_a > ps.F_star).mean())
        else:
            mean_x_active = float("nan")
            mean_y_active = float("nan")
            mean_epc_active = float("nan")
            mean_r_active = float("nan")
            mean_b_active = float("nan")
            fund_prevalence_active = 0.0
        ...
        if active_ids:
            ...
            n_enf = int(sum(1 for i in active_ids if self.people[i].is_enforcer))
            if n_enf > 0:
                n_enf_below = int(
                    sum(
                        1
                        for i in active_ids
                        if self.people[i].is_enforcer and self.people[i].cap < self.params.cap_to_enforcer
                    )
                )
                enforcer_cap_below_threshold_frac = float(n_enf_below / n_enf)
            else:
                enforcer_cap_below_threshold_frac = 0.0
        ...
        return dict(
            ...
            mean_x_active=mean_x_active,
            mean_y_active=mean_y_active,
            mean_epc_active=mean_epc_active,
            mean_r_active=mean_r_active,
            mean_b_active=mean_b_active,
            fund_prevalence_active=fund_prevalence_active,
            n_total=int(n_total),
            n_active=int(n_active),
            active_frac=float(n_active / n_total) if n_total > 0 else 0.0,
            ...
            enforcer_cap_below_threshold_frac=enforcer_cap_below_threshold_frac,
...
def infer_batch_root(outdir: Path) -> Path:
    outdir = outdir.resolve()
    for anc in [outdir] + list(outdir.parents):
        if anc.name.startswith("batch_"):
            return anc
    return outdir.parent


def ensure_batch_pip_freeze(batch_root: Path) -> Path:
    batch_root.mkdir(parents=True, exist_ok=True)
    freeze_path = batch_root / "pip_freeze.txt"
...
def resolve_git_commit(repo_root: Path) -> Any:
...
def write_provenance(outdir: Path, batch_root: Path, args: argparse.Namespace, model_path: Path, pip_freeze_path: Path):
    ...
    params_hash = hashlib.sha256(args_json.encode("utf-8")).hexdigest()

    repo_root = Path(__file__).resolve().parents[1]
    git_commit = resolve_git_commit(repo_root)
    pip_rel = os.path.relpath(str(pip_freeze_path), str(outdir))
    ...
    (outdir / "provenance.json").write_text(json.dumps(run_payload, indent=2) + "\n", encoding="utf-8")

    batch_payload_path = batch_root / "BATCH_PROVENANCE.json"
    if batch_payload_path.exists():
        ...
        batch_payload = {}
    batch_payload.setdefault("batch_root", str(batch_root))
    batch_payload.setdefault("model_file", model_path.name)
    batch_payload.setdefault("model_sha256", model_sha256)
    batch_payload.setdefault("pip_freeze_path", os.path.relpath(str(pip_freeze_path), str(batch_root)))
    batch_payload.setdefault("runs", {})
    run_rel = os.path.relpath(str(outdir), str(batch_root))
    batch_payload["runs"][run_rel] = {
        "seed": int(args.seed),
        "params_hash": params_hash,
        "timestamp_utc": timestamp_utc,
    }
    batch_payload_path.write_text(json.dumps(batch_payload, indent=2) + "\n", encoding="utf-8")
...
def main():
    ...
    batch_root = infer_batch_root(outdir)
    pip_freeze_path = ensure_batch_pip_freeze(batch_root)
    model_path = Path(__file__).resolve()

    model = ReligionABM(params)

    frames_dir = os.path.join(str(outdir), "frames")
    pos = None
    ...
    pd.DataFrame(rows).to_csv(os.path.join(str(outdir), "metrics.csv"), index=False)
    model.agent_summary(include_betweenness=args.compute_betweenness).to_csv(
        os.path.join(str(outdir), "agent_summary.csv"), index=False
    )
    write_provenance(outdir, batch_root, args, model_path, pip_freeze_path)
    if not args.no_movie:
        make_movie(frames_dir, os.path.join(str(outdir), "movie.mp4"), fps=args.fps)

    if args.no_movie:
        print(f"Done.\nMetrics: {os.path.join(str(outdir), 'metrics.csv')}")
    else:
        print(f"Done.\nMP4: {os.path.join(str(outdir), 'movie.mp4')}\nMetrics: {os.path.join(str(outdir), 'metrics.csv')}")
