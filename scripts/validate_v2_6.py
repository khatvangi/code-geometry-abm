...
def run_one(model_path: Path, outdir: Path, seed: int, alpha: float, delta: float, best: dict):
    cmd = [
        sys.executable,
        str(model_path),
        "--outdir",
...
def main():
    ...
    out_root = repo / "results" / "v2.6_validation"
    if out_root.exists():
    ...
    v26 = repo / "src" / "religion_fundamentalism_abm_v2_6.py"

    # Check 1: exact recovery on shared deterministic outputs for identical seed/params.
    c1_v25 = out_root / "check1_v25_seed11"
    c1_v26 = out_root / "check1_v26_seed11"
    run_one(v25, c1_v25, seed=11, alpha=0.0, delta=0.0, best=best)
    run_one(v26, c1_v26, seed=11, alpha=0.0, delta=0.0, best=best)
    m25 = pd.read_csv(c1_v25 / "metrics.csv")
    m26 = pd.read_csv(c1_v26 / "metrics.csv")
    shared_cols = [c for c in m25.columns if c in m26.columns]
    check1_metrics_equal = m25[shared_cols].equals(m26[shared_cols])
    a25 = pd.read_csv(c1_v25 / "agent_summary.csv")
    a26 = pd.read_csv(c1_v26 / "agent_summary.csv")
    check1_agents_equal = a25.equals(a26)
    check1_pass = bool(check1_metrics_equal and check1_agents_equal)

    # Check 2/3 on 5 seeds each.
    seeds = [1, 2, 3, 4, 5]
    rows = []
    conds = [
        ("baseline", 0.0, 0.0),
    ...
    for label, alpha, delta in conds:
        for seed in seeds:
            out = out_root / f"{label}_seed_{seed:02d}"
            run_one(v26, out, seed=seed, alpha=alpha, delta=delta, best=best)
            s = summarize_run(out)
            s.update({"condition": label, "seed": seed, "alpha": alpha, "delta": delta})
            rows.append(s)
    ...
    alpha = by.set_index("condition").loc["alpha_only"]
    delta = by.set_index("condition").loc["delta_only"]

    share_tol = 0.05
    check2_exit_lower = bool(alpha["median_exit_rate"] < base["median_exit_rate"])
    check2_shares_stable = bool(
        abs(alpha["median_top5_share"] - base["median_top5_share"]) <= share_tol
        and abs(alpha["median_enforcer_punish_share"] - base["median_enforcer_punish_share"]) <= share_tol
    )
    check2_pass = bool(check2_exit_lower and check2_shares_stable)

    check3_exit_lower = bool(delta["median_exit_rate"] < base["median_exit_rate"])
    check3_shares_stable = bool(
        abs(delta["median_top5_share"] - base["median_top5_share"]) <= share_tol
        and abs(delta["median_enforcer_punish_share"] - base["median_enforcer_punish_share"]) <= share_tol
    )
    check3_pass = bool(check3_exit_lower and check3_shares_stable)

    out = {
        "check1": {
            "shared_metrics_equal": check1_metrics_equal,
            "agent_summary_equal": check1_agents_equal,
            "pass": check1_pass,
        },
        "check2": {
            "baseline": {
            ...
            "alpha_only": {
                "median_exit_rate": float(alpha["median_exit_rate"]),
                "median_top5_share": float(alpha["median_top5_share"]),
                "median_enforcer_punish_share": float(alpha["median_enforcer_punish_share"]),
            },
        ...
        "check3": {
            ...
            "delta_only": {
                "median_exit_rate": float(delta["median_exit_rate"]),
                "median_top5_share": float(delta["median_top5_share"]),
                "median_enforcer_punish_share": float(delta["median_enforcer_punish_share"]),
            },
            "exit_lower": check3_exit_lower,
            "shares_stable": check3_shares_stable,
            "pass": check3_pass,
        },
