...
OUT_ROOT="${1:-runs/batch_v2_5a_hardening_$(date +%Y%m%d_%H%M%S)}"
...
for seed in $(seq -w 1 30); do
  ...
  mkdir -p "$outdir"
  echo "[$(date -Iseconds)] RUN v2.5a bestparams seed=${iseed}"
  python src/religion_fundamentalism_abm_v2_5a.py \
    --outdir "$outdir" \
    --steps 450 \
    --n 350 \
    --graph scale_free \
