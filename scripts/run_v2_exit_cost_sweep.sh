...
mkdir -p "$RUN_ROOT/exit_cost_sweep"
...
for v in $(seq 2 2 20); do
  exit_cost="$(awk -v x="$v" 'BEGIN { printf "%.2f", x/10.0 }')"
  for seed in $(seq 1 "$SEEDS"); do
    seed_str=$(printf "%02d" "$seed")
    outdir="$RUN_ROOT/exit_cost_sweep/exit_cost_${exit_cost}/seed_${seed_str}"
    mkdir -p "$outdir"

    echo "[$(date -Iseconds)] exit_cost_sweep exit_cost=${exit_cost} seed=${seed}" | tee -a "$RUN_ROOT/run_exit_cost_sweep.log"
    python "$ABM" \
      --outdir "$outdir" \
      --steps "$STEPS" \
      --n "$N" \
      ...
      --baseline_d0 0.18 \
      --no_movie \
      >> "$RUN_ROOT/run_exit_cost_sweep.log" 2>&1
  done
done
