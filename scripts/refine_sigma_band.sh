...
RUN_ROOT="${1:-}"
if [[ -z "$RUN_ROOT" ]]; then
  if [[ -f runs/_meta/last_batch_v1_phase_root.txt ]]; then
    RUN_ROOT="$(cat runs/_meta/last_batch_v1_phase_root.txt)"
  else
    echo "No run root provided and no runs/_meta/last_batch_v1_phase_root.txt found" >&2
    exit 1
  fi
fi
...
# If the detected interval is too narrow, widen to +-0.02 around center.
...
LOG="$RUN_ROOT/run_sigma_refine.log"
echo "[$(date -Iseconds)] Refining sigma in [$sigma_low, $sigma_high]" | tee -a "$LOG"
...
echo "[$(date -Iseconds)] Sigma refinement complete" | tee -a "$LOG"
printf '%s\n' "$RUN_ROOT"
