...
OUT_ROOT="${1:-runs/batch_v2_5_monopoly_$(date +%Y%m%d_%H%M%S)}"
...
for coeff in "${THREAT_COEFFS[@]}"; do
  ...
  for shock in "${SHOCKS[@]}"; do
    ...
    for seed in $(seq -w 1 5); do
      ...
      mkdir -p "$outdir"
      echo "[$(date -Iseconds)] RUN v2.5 sweep1 quota=${qfmt} coeff=${coeff_fmt} shock=${shock_fmt} seed=${iseed}"
      python src/religion_fundamentalism_abm_v2_5.py \
        --outdir "$outdir" \
        --steps 450 \
        --n 350 \
        --graph scale_free \
