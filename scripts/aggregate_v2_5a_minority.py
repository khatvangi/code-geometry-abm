...
def main():
    ...
    for run in sorted(run_dirs):
        ...
        rows.append({
            ...
            'final_prevalence_total': float(final.get('fund_prevalence', np.nan)),
            'final_prevalence_active': float(final.get('fund_prevalence_active', np.nan)),
    ...
    cols = [
        ...
        'final_exit_rate', 'final_prevalence_total', 'final_prevalence_active', 'max_punish_rate'
    ]
    ...
    memo = []
    prev_total_med = float(by_seed['final_prevalence_total'].median())
    prev_active_med = float(by_seed['final_prevalence_active'].dropna().median()) if by_seed['final_prevalence_active'].notna().any() else float('nan')

    memo.append('# RESULTS V2.5A MINORITY\n')
    ...
    memo.append(f'Median final prevalence (total): {prev_total_med:.3f}')
    memo.append(f'Median final prevalence (active): {prev_active_med:.3f}' if not math.isnan(prev_active_med) else 'Median final prevalence (active): nan')
    memo.append('')
    ...
    (summary_dir / 'RESULTS_V2_5A_MINORITY.md').write_text('\n'.join(memo) + '\n', encoding='utf-8')

    print(f'Wrote {by_seed_path}')
    print(f'Wrote {summary_path}')
    print(f'Wrote {summary_dir / "RESULTS_V2_5A_MINORITY.md"}')
