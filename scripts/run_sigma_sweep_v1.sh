...
if [[ -z "$RUN_ROOT" ]]; then
  ...
  RUN_ROOT="runs/batch_v1_phase_${TS}"
fi
mkdir -p "$RUN_ROOT" runs/_meta
printf '%s\n' "$RUN_ROOT" > runs/_meta/last_batch_v1_phase_root.txt

LOG="$RUN_ROOT/run_sigma_sweep_v1.log"

echo "[$(date -Iseconds)] Starting coarse sigma sweep at $RUN_ROOT" | tee -a "$LOG"
...
echo "[$(date -Iseconds)] Coarse sigma sweep complete" | tee -a "$LOG"
printf '%s\n' "$RUN_ROOT"
