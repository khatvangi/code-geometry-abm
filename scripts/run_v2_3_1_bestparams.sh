...
BEST_JSON="${1:-runs/batch_v2_3_search_20260213_043518/summary/BEST_PARAMS.json}"
OUT_ROOT="${2:-runs/batch_v2_3_1_minority_$(date +%Y%m%d_%H%M%S)}"
...
# Extract best parameters from JSON.
...
for seed in $(seq -w 1 30); do
  ...
  mkdir -p "$outdir"
  echo "[$(date -Iseconds)] RUN bestparams seed=${iseed}"
